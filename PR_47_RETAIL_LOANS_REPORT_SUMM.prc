-- PROCEDURE PR_47_RETAIL_LOANS_REPORT_SUMM (ARUNN_ADMIN)

  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARUNN_ADMIN"."PR_47_RETAIL_LOANS_REPORT_SUMM" (p_branch_code IN VARCHAR2,
...
BEGIN
  -- Construct filename
  l_filename := '47_RETAIL_LOANS_PER_PRODUCT_SUMM' || p_branch_code || '.csv';
  ...
  FOR rec IN (SELECT V12.PRODUCT_CODE, v12cnt, v14cnt
                ...
                LEFT JOIN (SELECT A.PRODUCT_CODE,
                                 count(A.ACCOUNT_NUMBER) v14cnt
                  ...
                  ON V14.PRODUCT_CODE = V12.PRODUCT_CODE) LOOP
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    END IF;
    RAISE;
END pr_47_RETAIL_LOANS_REPORT_SUMM;
/
/
