-- PROCEDURE PR_39_ACH_INCOMING_SUMM (ARUNN_ADMIN)

  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARUNN_ADMIN"."PR_39_ACH_INCOMING_SUMM" (p_branch_code IN VARCHAR2,
...
BEGIN
  -- Construct filename
  l_filename := '39_ACH_INCOMING_SUMM' || p_branch_code || '.csv';
...
select substr(V12.AC_ENTRY_REF_NO, 4, 4) AS SOURCE_REF,
  ...
  LEFT OUTER JOIN (SELECT A.TXN_REF_NO,
                          ...
                          A.TRANSFER_CCY,
                          A.INSTRUCTION_DATE
                     FROM integratedpp.PYZB_IN_TXN_DRIVER A
