-- PROCEDURE PR_21_UTILVSFACILITY_V14 (ARUNN_ADMIN)

  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARUNN_ADMIN"."PR_21_UTILVSFACILITY_V14" (p_dir IN VARCHAR2) IS
...
BEGIN
  -- Construct filename
  l_filename := '21_utilvsfacility_v14.csv';
  ...
  FOR rec IN (SELECT u.facility_id AS fac_id,
                ...
                FROM integratedpp.gezm_facility f
                JOIN (SELECT utils.limit_id AS facility_id,
                            SUM(convert_currency_v14(utils.Util_Ccy,
                                                     line.LINE_CURRENCY,
                                                     utils.Util_Amt)) AS line_utils,
                            SUM(convert_currency_v14(utils.Util_Ccy,
                                                     line.LINE_CURRENCY,
                                                     utils.Matured_Amt)) AS matured_utils
                       FROM integratedpp.Gezb_Utils utils
                       JOIN integratedpp.gezm_facility line
                ...
                LEFT JOIN integratedpp.gezm_liab liab
