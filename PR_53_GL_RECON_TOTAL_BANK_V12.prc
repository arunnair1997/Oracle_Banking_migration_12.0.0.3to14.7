-- PROCEDURE PR_53_GL_RECON_TOTAL_BANK_V12 (ARUNN_ADMIN)

  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARUNN_ADMIN"."PR_53_GL_RECON_TOTAL_BANK_V12" (p_dir IN VARCHAR2) IS
...
BEGIN
  -- Construct filename
  l_filename := '53_GL_RECON_TOTAL_BANK_V12.csv';
  ...
  FOR rec IN (SELECT B.branch_code,
                     ...
                     B.dr_mov_lcy,
                     B.cr_mov_lcy
                FROM GLTB_GL_BAL@fcubsv12 B, GLTM_GLMASTER@fcubsv12 M
               WHERE B.GL_CODE = M.GL_CODE
                 AND (B.PERIOD_CODE, B.FIN_YEAR) in
                     (select current_period, current_cycle
                        from sttm_branch@fcubsv12 sb
                       where sb.branch_code = '100')
                 AND M.RECORD_STAT = 'O'
                 AND M.AUTH_STAT = 'A'
                 and b.leaf = 'Y'
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    END IF;
    RAISE;
END PR_53_GL_RECON_TOTAL_BANK_V12;
/
/
