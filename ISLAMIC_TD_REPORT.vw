-- VIEW ISLAMIC_TD_REPORT (ARUNN_ADMIN)

  CREATE OR REPLACE FORCE EDITIONABLE VIEW "ARUNN_ADMIN"."ISLAMIC_TD_REPORT" ("ACC", "BRN", "CCY", "PROD", "ACCOUNT_CLASS", "TD_AMOUNT", "INT_START_DATE", "INTEREST_RATE", "LAST_IS_DATE", "ROLLOVER_TYPE", "MATURITY_DATE", "MATURITY_AMOUNT", "RULE", "INT_CALC_METHOD", "UDE_EFF_DT", "UDE_ID", "UDE_VALUE", "RATE_CODE", "UDE_VARIANCE", "AMOUNT_BLOCK_NO", "AMOUNT", "LOCK_IN_DAYS", "LOCK_IN_MONTHS", "LOCK_IN_YEARS", "RECORD_STAT", "CIF_SIG_ID", "JOINT_HOLDER_LIST", "FIELD_VAL_1", "FIELD_VAL_2", "FIELD_VAL_3", "FIELD_VAL_5", "FIELD_VAL_6") AS 
 ...
 (SELECT *
  ...
  SELECT ICU.ACC,
         ...
         FUF.FIELD_VAL_6
          ...
          LEFT JOIN INTEGRATEDPP.AMOUNT_BLOCK_RANKED AB
          ...
          LEFT JOIN INTEGRATEDPP.CSZM_FUNCTION_USERDEF_FIELDS FUF
           ...
           AND IC.ACC = SUBSTR(FUF.REC_KEY,
                               INSTR(FUF.REC_KEY, '~', 1, 1) + 1,
                               (INSTR(FUF.REC_KEY, '~', 1, 2) -
                               INSTR(FUF.REC_KEY, '~', 1, 1) - 1))
