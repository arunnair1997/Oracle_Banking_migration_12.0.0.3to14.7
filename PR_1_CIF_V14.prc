-- PROCEDURE PR_1_CIF_V14 (ARUNN_ADMIN)

  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARUNN_ADMIN"."PR_1_CIF_V14" (p_branch_code IN VARCHAR2,
  ...
  BEGIN
    -- Construct filename
    l_filename := 'pr_1_CIF_' || p_branch_code || '_v14.csv';
...
BEGIN
  ...
  FOR rec IN (SELECT CIF.CUSTOMER_NO,
                     ...
                     CIU.FIELD_VAL_21,
                     CIU.FIELD_VAL_22
                FROM INTEGRATEDPP.STZM_CUSTOMER CIF
                LEFT JOIN INTEGRATEDPP.STZM_CUST_PERSONAL CIP
                  ON CIF.CUSTOMER_NO = CIP.CUSTOMER_NO
                LEFT JOIN INTEGRATEDPP.CSZM_FUNCTION_USERDEF_FIELDS CIU
