-- PROCEDURE PR_7_ISLAMIC_TD_V12 (ARUNN_ADMIN)

  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARUNN_ADMIN"."PR_7_ISLAMIC_TD_V12" (
    ...
    FROM (
        ...
        FROM UBSPROD.ICTM_TD_DETAILS@FCUBSV12 ICTD
 
        WHERE ICTD.ACC IN (
 
            SELECT CUST_AC_NO FROM UBSPROD.STTM_CUST_ACCOUNT@FCUBSV12
...
Latest_ICU AS (
    ...
    FROM (
        ...
        FROM UBSPROD.ICTM_ACC_UDEVALS@FCUBSV12 ICU
...
Latest_AB AS (
    ...
    FROM (
        ...
        FROM UBSPROD.CATM_AMOUNT_BLOCKS@FCUBSV12 AB
...
Latest_ICLK AS (
    ...
    FROM (
        ...
        FROM UBSPROD.ICTM_ACC_BRKN_PR_CUSTOM@FCUBSV12 ICLK
        ...
        WHERE AJH.CUST_AC_NO IN (
            ...
            WHERE RECORD_STAT = 'O' AND AUTH_STAT = 'A'
              ...
              AND CUST_NO IN (
                  ...
                  SELECT CUSTOMER_NO FROM UBSPROD.STTM_CUSTOMER@FCUBSV12
...
Latest_FUF AS (
    ...
    FROM (
        ...
        FROM UBSPROD.CSTM_FUNCTION_USERDEF_FIELDS@FCUBSV12 FUF
...
Latest_INT AS (
    ...
    FROM (
        ...
        FROM UBSPROD.ICTM_ACC_UDEVALS@FCUBSV12 U
 
        JOIN UBSPROD.ICTM_PR_INT@FCUBSV12 P ON U.PROD = P.PRODUCT_CODE
 
        JOIN UBSPROD.ICTM_EXPR@FCUBSV12 E ON P.RULE = E.RULE_ID
...
Latest_TOT_INT AS (
    ...
    FROM (
        ...
        JOIN UBSPROD.ICTM_ACC@FCUBSV12 IC ON ICTD.ACC = IC.ACC AND ICTD.BRN = IC.BRN
...
FROM UBSPROD.ICTM_ACC@FCUBSV12 IC
...
LEFT JOIN UBSPROD.STTM_CUST_ACCOUNT@FCUBSV12 CUSAC
...
BEGIN
    -- open file
    l_filename := 'PR_7_ISLAMIC_TD_' || p_branch_code || '_V12.csv';
