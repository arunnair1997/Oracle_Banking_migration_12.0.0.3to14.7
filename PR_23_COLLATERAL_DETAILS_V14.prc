-- PROCEDURE PR_23_COLLATERAL_DETAILS_V14 (ARUNN_ADMIN)

  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARUNN_ADMIN"."PR_23_COLLATERAL_DETAILS_V14" (p_branch_code IN VARCHAR2,
...
BEGIN
  -- Construct filename
  l_filename := '23_Collateral_Details_' || p_branch_code || '_v14.csv';
  ...
  FOR rec IN (select liab.liab_no,
    ...
    l_line := rec.LIAB_NO || ',' || rec.COLLATERAL_TYPE || ',' ||
              ...
              rec.AVAILABLE_AMOUNT || ',' || rec.INT_RATE || ',' ||
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    END IF;
    RAISE;
END pr_23_Collateral_Details_v14;
/
/
