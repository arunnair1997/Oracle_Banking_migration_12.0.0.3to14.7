-- PROCEDURE PR_11_STANDING_INSTRUCTIONS_CONTRACTS_V12 (ARUNN_ADMIN)

  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARUNN_ADMIN"."PR_11_STANDING_INSTRUCTIONS_CONTRACTS_V12" (p_branch_code IN VARCHAR2,
...
BEGIN
  -- Construct filename
  l_filename := '11_STANDING_INSTRUCTIONS_CONTRACTS_' || p_branch_code ||
                '_V12.csv';
  ...
  FOR rec IN (WITH instr AS (
    SELECT I.BRANCH,
      ...
      JOIN UBSPROD.SITB_CONTRACT_MASTER@FCUBSV12 A
      ON I.INSTRUCTION_NO = A.INSTRUCTION_NO
      JOIN UBSPROD.CSTB_CONTRACT@FCUBSV12 B
...
SELECT i.BRANCH,
       ...
       m.CR_ACCOUNT,
       m.CR_ACC_CCY,
       agl.AC_GL_NO AS ACCOUNT,
       DECODE(agl.AC_OR_GL,'A','ACCOUNT','G','GL') AS ACCOUNT_TYPE,
  ...
  JOIN UBSPROD.CSTB_CONTRACT@FCUBSV12 c
   ...
   AND c.AUTH_STATUS = 'A'
  LEFT JOIN UBSPROD.STTB_ACCOUNT@FCUBSV12 agl
    ON (agl.AC_GL_NO = m.DR_ACCOUNT OR agl.AC_GL_NO = m.CR_ACCOUNT)
WHERE i.rn = 1
  AND i.BRANCH = p_branch_code
  AND M.SI_EXPIRY_DATE >  (SELECT today
                            FROM ubsprod.sttm_dates@fcubsv12
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    END IF;
    RAISE;
END PR_11_STANDING_INSTRUCTIONS_CONTRACTS_V12;
/
/
