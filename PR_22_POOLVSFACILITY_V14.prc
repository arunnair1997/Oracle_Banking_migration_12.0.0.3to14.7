-- PROCEDURE PR_22_POOLVSFACILITY_V14 (ARUNN_ADMIN)

  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARUNN_ADMIN"."PR_22_POOLVSFACILITY_V14" (p_branch_code IN VARCHAR2,
...
BEGIN
  -- Construct filename
  l_filename := '22_PoolvsFacility_' || p_branch_code || '_v14.csv';
  ...
  FOR rec IN (SELECT fac.id,
                     ...
                     liab.liab_name,
                     fac.line_currency,
                     fac.line_code || fac.line_serial facility,
                     mainline.line_code || mainline.line_serial mainline,
                ...
                JOIN integratedpp.gczm_pool pool
                  ON pool.liab_id = fac.liab_id
                JOIN integratedpp.gezb_pool_link gpl
                  ON gpl.facility_id = fac.id
                 AND gpl.liab_id = fac.liab_id
                 AND gpl.pool_id = pool.id
                LEFT JOIN integratedpp.gezm_facility mainline
