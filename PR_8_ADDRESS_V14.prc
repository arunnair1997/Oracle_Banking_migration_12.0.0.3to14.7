-- PROCEDURE PR_8_ADDRESS_V14 (ARUNN_ADMIN)

  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARUNN_ADMIN"."PR_8_ADDRESS_V14" (p_branch_code IN VARCHAR2,
...
BEGIN
  -- Construct filename
  l_filename := 'pr_8_address_' || p_branch_code || '_v14.csv';
  ...
  FOR rec IN (select CUST_AC_NO,
               ...
               where cust_ac_no in(select cust_ac_no from integratedpp.stzm_cust_account where record_stat = 'O' and auth_stat = 'A')
