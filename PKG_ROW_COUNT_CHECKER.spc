-- PACKAGE PKG_ROW_COUNT_CHECKER (ARUNN_ADMIN)

  CREATE OR REPLACE EDITIONABLE PACKAGE "ARUNN_ADMIN"."PKG_ROW_COUNT_CHECKER" AS
 
    /**
     * The main controller procedure. This is the only procedure you need to call.
     * It orchestrates the parallel execution of row count checks using DBMS_JOB.
     */
    PROCEDURE MAIN_CONTROLLER;
   
    /**
     * The worker procedure that processes a single row.
     * It is called by the background DBMS_JOB process.
     * MODIFIED: Made public by declaring it in the specification.
     */
    PROCEDURE EXECUTE_SINGLE_ROW(p_row_id IN ROWID);
